import type { UploadedFileItem } from '@/components/tiptap-templates/simple/simple-editor';
 ...
 */
/**
 * Flexible image upload handler supporting multiple upload action signatures
 * @param file - File to upload
 * @param uploadAction - Upload function (supports both mutation and legacy patterns)
...
export const handleImageUpload = async (
  ...
  try {
    // Detect upload action type and call appropriately
    let uploaded;

    // Check if it's a mutation function (React Query pattern)
    if (typeof uploadAction === 'function') {
      // Try calling with { file, config } pattern first (new pattern)
      try {
        uploaded = await uploadAction({
          file,
          config: {
        ...
        // If that fails, try legacy FormData pattern
        if (err?.message?.includes('uploadAction')) {
          const formData = new FormData();
          formData.append('file', file);

          uploaded = await uploadAction(formData, {
            onUploadProgress: (progressEvent: any) => {
              const total = progressEvent.total || file.size || 1;
              const progress = Math.round((progressEvent.loaded * 100) / total);
              onProgress?.({ progress });
    ...
    if ((error as any)?.code !== 'ERR_CANCELED' && !abortSignal?.aborted) {
